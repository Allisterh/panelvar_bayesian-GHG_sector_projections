...
if(user == "cluster") {
  ...
  nsave=500
  nburn=500 # very small number of burn-in, needs more.
}
...
if(user != "cluster") {
  ...
  # change to be an element of 1:10
  run <- 1
}
...
if(small == "TRUE") {
  ...
  Yraw <- Yraw[, grep("(\\.CO2$|\\.GDPpcPPP|\\.POP|\\.old_share$|\\.sec_share$)", names(Yraw))]
} else if(small == "FALSE") {
...
#####
# Create directories
dir.create("./02_output/09_results/NG_PVAR", showWarnings = FALSE)
folder <- paste0("./02_output/09_results/NG_PVAR/", em_data)
