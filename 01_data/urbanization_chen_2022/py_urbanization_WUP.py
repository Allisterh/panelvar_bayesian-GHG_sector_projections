...
data_all = pd.read_excel(pathin + 'WUP2018.xlsx') #Raw data of projrcting countries and areas
data_refe = pd.read_excel(pathin + 'reference.xlsx') #Raw data of reference countries and areas
...
for i in range(len(name_all)): #The data for every projecting countries and areas are fitted
  x = np.arange(-60,1,5)  
  y0 = np.array(data_all.iloc[i,1:14],dtype='float64')
...
for i in range(len(name_refe)): #The data for every reference countries and areas are fitted
  x = np.arange(-60,1,5)  
  y0 = np.array(data_refe.iloc[i,1:14],dtype='float64')
  beta = 90 if max(y0)<85 else 100
  y= np.log(1/y0-1/beta)-np.log(1/beta)-np.log(beta/y0[-1]-1)
  para_fit = leastsq(error, start, args=(x,y))
...
#year_pred: projecting year
  ...
  for i in range(len(name_all)):
    ...
    for j in range(len(name_refe)):
      refe_interval = Interval(data_refe.iloc[j,1:14].min(),data_refe.iloc[j,1:14].max())
  ...
  for i in range(len(name_all)):
    ...
    for j in range(len(refe)):
      ...
      x = np.arange(-start_yr*5,2010-1950+1-start_yr*5,5)
    ...
    x = np.arange(-year_base+1950,1,5)
    y0 = np.array(data_all.iloc[i,1:int((year_base-1950)/5+1+1)],dtype='float64')#
    beta = 90 if max(y0)<85 else 100
    y= np.log(1/y0-1/beta)-np.log(1/beta)-np.log(beta/y0[-1]-1)
    para_fit = leastsq(error, start, args=(x,y))
...
for i in range(18):
  smy0 = urban_predict(data_all,data_refe,2010+i*5,2015+i*5,level=1)
  data_all = data_all.iloc[:,0:(13+1+i)]
  data_all[2015+i*5] = smy0[:,0]


  result_SD = pd.DataFrame(smy0[:,1], columns = [2015+i*5])
  results_SD = pd.concat([results_SD,result_SD],axis=1)
 
  bat(i,18)
print (time.strftime('%Y-%m-%d %H:%M:%S'))  

data_all.to_excel(pathin + '0621urban_pred_fast0.85.xls')
results_SD.to_excel(pathin + '0621urban_pred_fast0.85_SD.xls')




results_SD2 = pd.DataFrame(np.zeros((country_num,1)), columns = [2010]) 

for i in range(18):
  smy0 = urban_predict(data_all,data_refe,2010+i*5,2015+i*5,level=2)
  data_all = data_all.iloc[:,0:(13+1+i)]
  data_all[2015+i*5] = smy0[:,0]
...
data_all.to_excel(pathin + '0621urban_pred_middle0.85.xls')
results_SD2.to_excel(pathin + '0621urban_pred_middle0.85_SD.xls')




results_SD3 = pd.DataFrame(np.zeros((country_num,1)), columns = [2010]) 

for i in range(18):
  smy0 = urban_predict(data_all,data_refe,2010+i*5,2015+i*5,level=3)
  data_all = data_all.iloc[:,0:(13+1+i)]
  data_all[2015+i*5] = smy0[:,0]
...
data_all.to_excel(pathin + '0621urban_pred_low0.85.xls')
results_SD3.to_excel(pathin + '0621urban_pred_low0.85_SD.xls')
